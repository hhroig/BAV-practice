...
La opción de arriba no nos vale porque `name` no identifica de forma única a cada fila. Si pasamos los nombres a las columnas:
...
Observamos que algunos outliers implícitos son ahora explícitos.
...
Por otro lado, `fill` rellena los espacios en blanco de acuerdo al criterio que se fije. Por defecto, rellena con `NA` si no hay suficientes elementos como para separar en la cantidad de columnas que hemos pedido. Podemos variar este comportamiento con los argumentos `fill = "left"` (rellenar a la izquierda con `NA`s) o `fill = "right"` (rellenar a la derecha con `NA`s), sin emitir *Warnings*:
...
**R/** Rellena los faltantes con el valor no faltante anterior.
